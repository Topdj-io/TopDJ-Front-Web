...
import { WorkDetailType, WorkType ,newWorkDetailType} from 'types/work'
      ...
      <Grid spacing={4} container>
        <Grid item xs container alignItems="center">
          {data?.work.type === WorkType.AUDIO  &&(
            <Audio className={classes.audio} src={audioUrl} poster={data?.work.path} />
          )}
          ...
          {data.work.id ? (
            <div className={classes.infoContainer}>
              <Grid
                container
                onClick={() => {
                  goToAuthorDetail(data.author?.id)
                }}
                alignItems="center"
                className={classes.djContainer}
              >
                <Grid item>
                  <img  src={data?.author?.avatar} className={classes.portrait} alt="" />
                </Grid>
                <Grid item marginLeft={1.2}>
                  <div className={classes.djName}>{`${data?.author?.first_name} ${data?.author?.last_name}`}</div>
                </Grid>
              </Grid>
              ...
              {typeS!=='3'&&(<div className={classes.btnBox}>
