...
class DragAcr {
  ...
  initParam(param) {
    ...
    this.radius = this.width / 2 - slider //滑动路径半径
    this.initCanvas(el)
  ...
  draw(value) {
    ...
    this.ctx.arc(this.P.x, this.P.y, this.slider, 0, Math.PI * 2, false) // 绘制滑块内侧
  ...
  OnMouseMove(evt) {
    ...
    let val =
      ((radian - (radian > this.startDeg ? this.startDeg : -this.residueDeg)) / (this.endDeg - this.startDeg)) * 100
